...
def hello(event, context):
    ...
    for s_dir in scraper_dirs:
        ...
        if keys:
            ...
            success = upload_file(zipped, BUCKET_NAME, object_name=s_dir + "/" +  zipped.name)
            ...
            if success:
                ...
                for key in keys:
                    ...
                    print(f"Deleted key: {key}. Delete response: {jq(DELETE_FILTER).transform(delete_resp)}")
        ...
        if empty_file_keys:
            for key in empty_file_keys:
                ...
                print(f"Deleted empty key: {key}. Delete response: {jq(DELETE_FILTER).transform(delete_resp)}")
...
def extract_keys(response: dict, jq_filter: str=NON_EMPTY_FILEKEYS) -> List[Path]:
    ...
    keys = [
        Path(key) for key in key_strings
    ]
    return keys
...
def zip_files(files: List[Path], destination_folder: Path):
    ...
    file_sizes = []
    start = time.time()
    with zipfile.ZipFile(zip_dst, mode="w", compression=zipfile.ZIP_DEFLATED, compresslevel=9) as f_out:
    ...
    print(f'Zipped file: {zip_dst} Elasped time: {(time.time() - start) / 1000:.1f} ms. Space savings: {(1 - zip_size / sum(file_sizes))*100:.1f} percent')
    return zip_dst
