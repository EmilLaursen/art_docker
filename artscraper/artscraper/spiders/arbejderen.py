...
class ArbejderenSpider(scrapy.Spider):
    ...
    def parse_startpage(self, response):
        link_css = (
            ".field-content a::attr(href)"
            if "blogs" in response.url
            else self.start_page_links
        )
        self.logger.info(
            f'blogs in response.url? {"blogs" in response.url}. Response: {response.url}'
        )
    ...
    def is_blog_url(self, url):
        return (
            self._get_section(url) == "blog-indlæg"
