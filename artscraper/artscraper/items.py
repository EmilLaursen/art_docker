...
class ArtscraperItem(scrapy.Item):
    ...
    authors = Field(input_processor=MapCompose(str.strip, filter_empty))
    section = Field(
        input_processor=Compose(
            MapCompose(str.strip, filter_empty), remove_section_garbage
        )
    )
    alt_authors = Field(input_processor=MapCompose(str.strip, filter_empty))
    date = Field(input_processor=MapCompose(str.strip, filter_empty, extract_datetime))
    sub_title = Field(input_processor=MapCompose(str.strip, filter_empty))
